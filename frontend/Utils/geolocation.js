...
const MAP_DEFAULT_CENTER_LATITUDE =
  process.env.REACT_APP_MAP_DEFAULT_CENTER_LATITUDE;
const MAP_DEFAULT_CENTER_LONGITUDE =
  process.env.REACT_APP_MAP_DEFAULT_CENTER_LONGITUDE;
...
export const getDefaultCenter = () => {
  ...
  return {
    lat: parseFloat(MAP_DEFAULT_CENTER_LATITUDE),
    lng: parseFloat(MAP_DEFAULT_CENTER_LONGITUDE),
  };
};
    ...
    .then(
      (response) => {
        ...
        if (response && response.length > 0) {
          ...
          geoLocation.error =
            "Response from leaflet-geosearch was incorrectly formed";
        }
        return geoLocation;
      },
      (error) => {
...
/**
 * Get address information from a latitude and logitude.
 * @param {Number} lat
 * @param {Number} lng
 */
export const getGeolocationFromLatLng = async (lat, lng) => {
  const response = await new OpenStreetMapProvider({
    params: {
      lat,
      lon: lng,
    },
